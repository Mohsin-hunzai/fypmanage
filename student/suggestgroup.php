<?php
  include '../dbcon.php';
  include '../header.php';
  $regNo = $_SESSION['id'];
?>
<style>
        .heading
 {
   font-size:24px;
   font-weight:bold;
   text-transform:uppercase;
 }
    </style>
<!-- Page Container -->
<div class="w3-container w3-content" style="max: width 80%;margin-top:80px">
  <div class="w3-row">
        <div class="w3-row-padding w3-container">
          <div class="w3-row-padding">
           <div class="w3-col m12">
            <div class="w3-card-2 w3-white">
              <div class="w3-container w3-padding">
                <h3 class="w3-center heading">SUGGEST GROUP</h3>
              </div>
            </div>
          </div>
        </div>
         <br />  
        <div class="w3-row-padding">
          <div class="w3-col m12">
            <div class="w3-card-2 w3-white">  
              <div class="w3-container w3-padding"> 
                <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST" class="" >           
                 <h4>Enter the registration numbers in the fields below</h4>
                  <br />               
                <div class="w3-row">
                  <div class="w3-col m6">                 
                    <select  class="w3-select"  name="fMember" >
                      <?php
                        
                        // $sql="SELECT * FROM student where regNo!='$regNo' ORDER BY regNO ASC";
                        $sql="SELECT * FROM student grp where regNo!='$regNo'  AND `regNO` NOT IN
                           (SELECT `regNO` FROM `grp` ) AND `regNO` NOT IN (SELECT `mRegNo` FROM `grp` ) 
                           ORDER BY regNO ASC";
                        // $sql="SELECT   * FROM (SELECT * from student  union all
                        // SELECT * FROM grp) ";

                       
                        $result=  mysqli_query($dbcon, $sql)
                        
                        ?>
                      <option>
                      <?php
                      while($row = mysqli_fetch_assoc($result))
                      {
                        $category= $row['regNo'];
                        ?>
                      <option value="<?php echo $category; ?>"><?php echo $category;?></option>
                      <?php
                      }
                      ?>
                    </select>
                  </div> 
                </div>
                <br />
                <br />
              <button type="submit" name="submit"  class="w3-padding w3-btn w3-blue">Submit Suggestion</button>
            </form>
             <br />  <!-- check div end -->
            <?php

              if (isset($_POST['submit'])) 
              {
                $fMember=$_POST['fMember'];
                // $fMember=$_POST['fMember'];
                // $fMember = mysqli_real_escape_string($dbcon,$_POST['fMember']);
                // $sMember = mysqli_real_escape_string($dbcon,$_POST['sMember']);

                $suggestion = mysqli_query($dbcon, "INSERT into suggestedgroup(fMember, tMember, approval) VALUES ('$fMember','$regNo', 'waiting') ") or die(mysqli_error($dbcon));
                if ($suggestion) {
                  ?>
              <script> alert("Your group suggestion has been recorded"); </script>
              <!-- <script>document.location='index.php'</script>; -->
                <?php  } else { ?>
              <script> alert("Sorry, your suggestion failed"); </script>
              <?php
                }
              }
          ?>

          </div>
        </div>
    </div>
  </div>
      </div>
    <br />
      
 
    
  <!-- End Grid -->
  <!-- check div end -->
  </div>
  
<!-- End Page Container -->
</div>
<br>

<!-- Footer -->
<?php
  include '../footer.php';
?>


      <br />
    </div>    


<br>

<script>
function suggestGroup() {
  var xmlhttp = new XMLHttpRequest();
  xmlhttp.onreadystatechange = function() {
  if (this.readyState == 4 && this.status == 200) {
                document.getElementById("main").innerHTML = this.responseText;
            }
        };
        xmlhttp.open("GET", "submitgroup.php", true);
        xmlhttp.send();
    }
}
function showMember(str) {
    if (str.length == 0) { 
        document.getElementById("member1").innerHTML = "";
        return;
    } else {
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                document.getElementById("member1").innerHTML = this.responseText;
            }
        };
        xmlhttp.open("GET", "../getmember.php?q=" + str, true);
        xmlhttp.send();
    }
}
function showMember2(str) {
    if (str.length == 0) { 
        document.getElementById("member2").innerHTML = "";
        return;
    } else {
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                document.getElementById("member2").innerHTML = this.responseText;
            }
        };
        xmlhttp.open("GET", "../getmember.php?q=" + str, true);
        xmlhttp.send();
    }
}
/*function showMember3(str) {
    if (str.length == 0) { 
        document.getElementById("member3").innerHTML = "";
        return;
    } else {
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                document.getElementById("member3").innerHTML = this.responseText;
            }
        };
        xmlhttp.open("GET", "../getmember.php?q=" + str, true);
        xmlhttp.send();
    }
}*/
</script>
